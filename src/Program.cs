...
using AzureSemanticSearchWebApi;
...
builder.Services.AddOptions<AzureConfiguration>()
    .Bind(builder.Configuration.GetSection(AzureConfiguration.Key))
    .ValidateDataAnnotations()
    .ValidateOnStart();

builder.Services.AddSingleton(provider =>
{
    var azureConfiguration = provider.GetRequiredService<IOptions<AzureConfiguration>>().Value;
    var apiCredential = new AzureKeyCredential(azureConfiguration.ApiKey);
    var adminClient = new SearchIndexClient(azureConfiguration.ServiceEndpoint, apiCredential);
    return adminClient;
});
...
{
    var azureConfiguration = provider.GetRequiredService<IOptions<AzureConfiguration>>().Value;
    var adminClient = provider.GetRequiredService<SearchIndexClient>();
    return adminClient.GetSearchClient(azureConfiguration.IndexName);
});
